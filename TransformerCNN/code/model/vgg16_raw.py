...
# 1. CONFIGURATION
# =============================================================================
DATASET_PATH = "/home/eecommulab/Desktop/bee/dataset"
OUTPUT_PATH = "/home/eecommulab/Desktop/bee/output/baseline_vgg16"

MODEL_NAME = "baseline_vgg16"
...
# Original VGG16 output names preserved for compatibility.
OLD_HISTORY_CSV_PATH = os.path.join(OUTPUT_PATH, "training_history_vgg16.csv")
OLD_RESULTS_CSV_PATH = os.path.join(OUTPUT_PATH, "baseline_vgg16_results.csv")
...
class BaselineDataset(Dataset):
    def __init__(self, file_list, labels, transform=None):
        self.image_paths = np.asarray(file_list)
...
def get_all_data(root_dir):
    ...
    print(f"\nChecking data path: {root_dir}")
    if not os.path.exists(root_dir):
        raise FileNotFoundError(f"Dataset directory does not exist: {root_dir}")

    # Check for train/val/test structure first.
    found_structure = False
    for subset in ["train", "val", "test"]:
        for idx, cls_name in enumerate(classes):
            folder_path = os.path.join(root_dir, subset, cls_name)
            if os.path.exists(folder_path):
                images = glob(os.path.join(folder_path, "*"))
                valid_imgs = [f for f in images if f.lower().endswith(valid_ext)]
                if valid_imgs:
                    found_structure = True
                    all_paths.extend(valid_imgs)
                    all_labels.extend([idx] * len(valid_imgs))

    # Check for flat NORMAL/PNEUMONIA folder structure.
    if not found_structure:
        print("train/val/test structure not found. Checking direct class folders...")
        for idx, cls_name in enumerate(classes):
    ...
    if len(all_paths) == 0:
        raise RuntimeError("No images found. Expected train/val/test/NORMAL/PNEUMONIA or NORMAL/PNEUMONIA folders.")

    all_paths = np.asarray(all_paths)
    all_labels = np.asarray(all_labels).astype(int)

    # Sort for stable reproducibility before CV shuffle.
    order = np.argsort(all_paths)
    all_paths = all_paths[order]
    all_labels = all_labels[order]

    print(f"Loaded {len(all_paths)} images.")
    for idx, cls_name in enumerate(classes):
        print(f" - {cls_name}: {(all_labels == idx).sum()}")
...
all_paths, all_labels, CLASSES = get_all_data(DATASET_PATH)
NUM_CLASSES = len(CLASSES)
if NUM_CLASSES != 2:
    raise ValueError(f"This script expects binary classification, but found classes: {CLASSES}")

positive_label = CLASSES.index("PNEUMONIA") if "PNEUMONIA" in CLASSES else 1
...
class BaselineVGG16(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.raw_feature_dim = 512
        self.classifier = nn.Sequential(
            nn.Dropout(0.3),
            nn.Linear(self.raw_feature_dim, 256),
...
experiment_config = {
    ...
    "augmentation": {
        ...
        "random_horizontal_flip": USE_AUGMENTATION,
        "random_rotation_degrees": 15 if USE_AUGMENTATION else 0,
        "details": "Original VGG16 code used resize, horizontal flip, rotation, tensor conversion, and ImageNet normalization.",
    },
    "normalization": {
...
glcm_config = {
    ...
    "model_name": MODEL_NAME,
    "note": "Raw-image VGG16 baseline does not compute GLCM maps. Fill real values in GLCM/fusion scripts.",
    "image_size": list(IMG_SIZE),
...
model_config = {
    ...
    "backbone": "VGG16",
    "pretrained_weights": "torchvision.models.VGG16_Weights.IMAGENET1K_V1",
    "num_classes": NUM_CLASSES,
    "classes": CLASSES,
    "positive_label": int(positive_label),
    "classifier": "GlobalAveragePooling -> Dropout(p=0.3) -> Linear(512,256) -> ReLU -> Linear(256,num_classes)",
    "fusion": {
        "used_in_this_model": False,
...
for fold, (train_idx, val_idx) in enumerate(splits, start=1):
    ...
    train_subset = BaselineDataset(all_paths[train_idx], all_labels[train_idx], transform=train_transform)
    val_subset = BaselineDataset(all_paths[val_idx], all_labels[val_idx], transform=val_transform)
...
complexity_model = BaselineVGG16(num_classes=NUM_CLASSES).to(DEVICE)
...
fusion_df["note"] = "single_raw_branch_vgg16_baseline_placeholder_not_real_fusion"
...
branch_df["note"] = "raw-only VGG16 baseline; texture/fusion probabilities must come from fusion script"
...
if GENERATE_XAI:
    ...
    xai_model = BaselineVGG16(num_classes=NUM_CLASSES).to(DEVICE)
    xai_model.load_state_dict(torch.load(MODEL_SAVE_PATH.format(1), map_location=DEVICE))
    xai_model.eval()

    # VGG16 last convolution layer is features[28].
    # XAI uses fold-1 checkpoint for representative examples only.
    gradcam = GradCAM(xai_model, xai_model.backbone[28])
    ...
    for idx, row in selected_cases.iterrows():
        try:
            ...
            safe_stem = os.path.splitext(os.path.basename(image_path))[0].replace(" ", "_")
