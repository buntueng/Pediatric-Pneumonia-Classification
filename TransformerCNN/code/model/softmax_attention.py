...
OUTPUT_PATH = "/home/eecommu06/Desktop/Bee/Pediatric_Pneumonia/output/fusion_attention"

MODEL_NAME = "fusion_softmax_attention_effnetb0_resnet18_glcm"
...
OLD_HISTORY_CSV_PATH = os.path.join(OUTPUT_PATH, "training_history_fusion.csv")
OLD_RESULTS_CSV_PATH = os.path.join(OUTPUT_PATH, "fusion_results.csv")
...
class SoftmaxAttention(nn.Module):
    """Feature-wise softmax attention inside one branch.

    The weights sum to 1 across features within the same branch. They should not
    be interpreted as scalar raw-vs-texture fusion gates.
    """
...
class FusionModel(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.raw_dim = 1280
        self.raw_attention = SoftmaxAttention(self.raw_dim)
        ...
        self.texture_attention = SoftmaxAttention(self.texture_dim)

        # Attended feature concatenation + MLP classifier.
        self.mlp_classifier = nn.Sequential(
            nn.Linear(self.raw_dim + self.texture_dim, 512),
            nn.ReLU(),
            nn.Dropout(0.3),
    ...
    def forward(self, raw, glcm, return_features=False):
        ...
        raw_attended, raw_attn_weights = self.raw_attention(raw_vec, return_weights=True)
        texture_attended, texture_attn_weights = self.texture_attention(texture_vec, return_weights=True)
        fused_vec = torch.cat((raw_attended, texture_attended), dim=1)
        logits = self.mlp_classifier(fused_vec)
        if return_features:
            return (
                logits,
                raw_vec,
...
experiment_config = {
    ...
    "augmentation": {
        "used": False,
        "details": "Original softmax.py used resize + tensor conversion + ImageNet normalization only.",
        "rotation_degrees": 0,
        "translation": 0,
        "brightness_jitter": 0,
        "contrast_jitter": 0,
...
glcm_config = {
    ...
    "compute_glcm_after_resize": "not_specified_in_original_softmax_py",
    "gray_levels": "not_specified_in_original_softmax_py",
    "window_size": "not_specified_in_original_softmax_py",
    "distances": "not_specified_in_original_softmax_py",
    "angles_degrees": "not_specified_in_original_softmax_py",
    "offsets": "not_specified_in_original_softmax_py",
    "direction_aggregation": "not_specified_in_original_softmax_py",
    "entropy_formula": "not_specified_in_original_softmax_py",
    "border_handling": "not_specified_in_original_softmax_py",
...
model_config = {
    ...
    "raw_branch": {
        ...
        "output_dim": 1280,
        "attention": "SoftmaxAttention: Linear(1280,640) -> ReLU -> Linear(640,1280) -> Softmax(dim=1)",
    },
    "texture_branch": {
        "backbone": "ResNet-18",
        ...
        "output_dim": 512,
        "attention": "SoftmaxAttention: Linear(512,256) -> ReLU -> Linear(256,512) -> Softmax(dim=1)",
    },
    "fusion": {
        "type": "branch_wise_feature_softmax_attention_then_concatenation",
        "attention_or_gate_used": True,
        "attention_position": "after each branch global feature vector and before concatenation",
        "fusion_vector_dim": 1792,
        "gate_network": "not a two-branch scalar gate; independent feature-wise softmax attention inside each branch",
        "fusion_weight_type": "feature_wise_softmax_weights_within_each_branch; branch summaries are derived from attended feature norms",
        "se_block_position": "not_applicable_SE_not_used_in_this_softmax_attention_script",
        "note": "w_raw and w_texture saved in fusion_weights_test.csv are normalized attended feature-norm ratios, not scalar softmax gate outputs.",
    },
    ...
    with torch.no_grad():
        for raw, glcm, labels, raw_paths, glcm_paths in loader:
            ...
            (
                ...
                raw_attn_weights,
                texture_attn_weights,
            ) = model(raw, glcm, return_features=True)
            probs = torch.softmax(outputs, dim=1).detach().cpu().numpy()
            ...
            raw_attended_norm = torch.linalg.vector_norm(raw_attended, ord=2, dim=1).detach().cpu().numpy()
            texture_attended_norm = torch.linalg.vector_norm(texture_attended, ord=2, dim=1).detach().cpu().numpy()
            fused_norm = torch.linalg.vector_norm(fused_vec, ord=2, dim=1).detach().cpu().numpy()

            denom_raw = raw_norm + texture_norm + 1e-12
            raw_norm_ratio = raw_norm / denom_raw
            texture_norm_ratio = texture_norm / denom_raw

            denom_att = raw_attended_norm + texture_attended_norm + 1e-12
            softmax_raw_attended_norm_weight = raw_attended_norm / denom_att
            softmax_texture_attended_norm_weight = texture_attended_norm / denom_att

            raw_attn_np = raw_attn_weights.detach().cpu().numpy()
            texture_attn_np = texture_attn_weights.detach().cpu().numpy()
            eps = 1e-12
            raw_attn_entropy = -(raw_attn_np * np.log(raw_attn_np + eps)).sum(axis=1)
            texture_attn_entropy = -(texture_attn_np * np.log(texture_attn_np + eps)).sum(axis=1)
            ...
            for i in range(len(labels)):
                row = {
                    ...
                    "raw_attended_feature_l2_norm": float(raw_attended_norm[i]),
                    "texture_attended_feature_l2_norm": float(texture_attended_norm[i]),
                    ...
                    "softmax_raw_attended_norm_weight": float(softmax_raw_attended_norm_weight[i]),
                    "softmax_texture_attended_norm_weight": float(softmax_texture_attended_norm_weight[i]),
                    "softmax_raw_attention_mean": float(raw_attn_np[i].mean()),
                    "softmax_texture_attention_mean": float(texture_attn_np[i].mean()),
                    "softmax_raw_attention_max": float(raw_attn_np[i].max()),
                    "softmax_texture_attention_max": float(texture_attn_np[i].max()),
                    "softmax_raw_attention_entropy": float(raw_attn_entropy[i]),
                    "softmax_texture_attention_entropy": float(texture_attn_entropy[i]),
                }
...
fusion_df = predictions_df[[
    ...
    "raw_feature_l2_norm", "texture_feature_l2_norm", "raw_attended_feature_l2_norm", "texture_attended_feature_l2_norm",
    "fused_feature_l2_norm", "raw_feature_norm_ratio", "texture_feature_norm_ratio",
    "softmax_raw_attended_norm_weight", "softmax_texture_attended_norm_weight",
    "softmax_raw_attention_mean", "softmax_texture_attention_mean",
    "softmax_raw_attention_max", "softmax_texture_attention_max",
    "softmax_raw_attention_entropy", "softmax_texture_attention_entropy",
]].copy()
fusion_df["w_raw"] = fusion_df["softmax_raw_attended_norm_weight"]
fusion_df["w_texture"] = fusion_df["softmax_texture_attended_norm_weight"]
fusion_df["fusion_type"] = "branch_wise_softmax_feature_attention_then_concat"
fusion_df["case_type"] = add_case_type(fusion_df)
fusion_df["note"] = "w_raw/w_texture are normalized attended feature-norm ratios. The model uses feature-wise softmax attention inside each branch, not a scalar two-branch gate."
...
branch_df = fusion_df[[
    ...
    "raw_feature_l2_norm", "texture_feature_l2_norm", "raw_attended_feature_l2_norm", "texture_attended_feature_l2_norm",
    "raw_feature_norm_ratio", "texture_feature_norm_ratio", "w_raw", "w_texture",
    "softmax_raw_attention_max", "softmax_texture_attention_max",
    "softmax_raw_attention_entropy", "softmax_texture_attention_entropy",
]].copy()
...
branch_df["note"] = "This model has no separate raw-only or texture-only classification heads. Use raw/GLCM baseline scripts for branch probabilities."
