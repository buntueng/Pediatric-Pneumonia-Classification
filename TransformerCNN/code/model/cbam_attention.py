...
OUTPUT_PATH = "/home/eecommu06/Desktop/Bee/Pediatric_Pneumonia/output/fusion_cbam"

MODEL_NAME = "fusion_cbam_effnetb0_resnet18_glcm"
...
OLD_HISTORY_CSV_PATH = os.path.join(OUTPUT_PATH, "training_history_cbam.csv")
OLD_RESULTS_CSV_PATH = os.path.join(OUTPUT_PATH, "cbam_results.csv")
...
class CBAMBlock(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        x, channel_attn = self.ca(x, return_attention=True)
        x, spatial_attn = self.sa(x, return_attention=True)
        if return_attention:
...
class FusionModel(nn.Module):
    def __init__(self, num_classes=2):
        ...
        # Spatial feature-map fusion + CBAM attention.
        self.fusion_dim = self.raw_dim + self.texture_dim
        self.cbam = CBAMBlock(self.fusion_dim, ratio=16, kernel_size=7)
    ...
    def forward(self, raw, glcm, return_features=False):
        ...
        attended_map, channel_attn, spatial_attn = self.cbam(fused_map, return_attention=True)
        ...
        if return_features:
            ...
            # CBAM is channel-wise, not a two-branch softmax gate. For auditing,
            # average channel attention over the raw channels and the texture channels,
            # then normalize these two averages to sum to 1.
            channel_attn_flat = channel_attn.flatten(1)   # [B, 1792]
            raw_channel_mean = channel_attn_flat[:, :self.raw_dim].mean(dim=1)
            texture_channel_mean = channel_attn_flat[:, self.raw_dim:].mean(dim=1)
            denom = raw_channel_mean + texture_channel_mean + 1e-12
            cbam_raw_weight = raw_channel_mean / denom
            cbam_texture_weight = texture_channel_mean / denom
            cbam_spatial_mean = spatial_attn.flatten(1).mean(dim=1)
...
experiment_config = {
    ...
    "augmentation": {
        "used": False,
        "details": "Original cbam.py used resize + tensor conversion + ImageNet normalization only.",
        "rotation_degrees": 0,
        "translation": 0,
        "brightness_jitter": 0,
        "contrast_jitter": 0,
...
glcm_config = {
    ...
    "compute_glcm_after_resize": "not_specified_in_original_cbam_py",
    "gray_levels": "not_specified_in_original_cbam_py",
    "window_size": "not_specified_in_original_cbam_py",
    "distances": "not_specified_in_original_cbam_py",
    "angles_degrees": "not_specified_in_original_cbam_py",
    "offsets": "not_specified_in_original_cbam_py",
    "direction_aggregation": "not_specified_in_original_cbam_py",
    "entropy_formula": "not_specified_in_original_cbam_py",
    "border_handling": "not_specified_in_original_cbam_py",
...
model_config = {
    ...
    "fusion": {
        "type": "spatial_feature_map_concatenation_followed_by_CBAM",
        "attention_or_gate_used": True,
        "fusion_map_channels": 1792,
        "cbam_position": "after concatenating EfficientNet-B0 raw feature maps and ResNet-18 GLCM feature maps, before global average pooling",
        "channel_attention": "average_pool_and_max_pool -> shared 1x1 MLP/conv -> sigmoid channel attention",
        "spatial_attention": "channel average/max maps -> 7x7 convolution -> sigmoid spatial attention",
        "gate_network": "not_used_two_scalar_gate; CBAM channel/spatial attention is used instead",
        "fusion_weight_type": "CBAM channel-attention summary; w_raw and w_texture are branch-wise averages normalized to sum to 1 for auditing",
        "se_block_position": "not_applicable_CBAM_used_instead_of_SE",
        "note": "w_raw and w_texture saved in fusion_weights_test.csv are derived from CBAM channel attention averages over raw and texture channel groups; they are not softmax gate outputs.",
    },
    "classifier": "AdaptiveAvgPool2d(1) -> Flatten -> Linear(1792,512) -> ReLU -> Dropout(0.3) -> Linear(512,num_classes)",
}
...
def evaluate_with_predictions(model, loader, fold, threshold=0.5):
    ...
    with torch.no_grad():
        for raw, glcm, labels, raw_paths, glcm_paths in loader:
            raw = raw.to(DEVICE)
            glcm = glcm.to(DEVICE)
            outputs, raw_vec, texture_vec, fused_vec, cbam_raw_w, cbam_texture_w, cbam_spatial_mean = model(raw, glcm, return_features=True)
            probs = torch.softmax(outputs, dim=1).detach().cpu().numpy()
            ...
            cbam_raw_w_np = cbam_raw_w.detach().cpu().numpy()
            cbam_texture_w_np = cbam_texture_w.detach().cpu().numpy()
            cbam_spatial_mean_np = cbam_spatial_mean.detach().cpu().numpy()
            ...
            for i in range(len(labels)):
                row = {
                    ...
                    "cbam_raw_channel_weight": float(cbam_raw_w_np[i]),
                    "cbam_texture_channel_weight": float(cbam_texture_w_np[i]),
                    "cbam_spatial_attention_mean": float(cbam_spatial_mean_np[i]),
                }
...
fusion_df = predictions_df[[
    ...
    "cbam_raw_channel_weight", "cbam_texture_channel_weight", "cbam_spatial_attention_mean"
]].copy()
fusion_df["w_raw"] = fusion_df["cbam_raw_channel_weight"]
fusion_df["w_texture"] = fusion_df["cbam_texture_channel_weight"]
fusion_df["fusion_type"] = "CBAM_after_spatial_feature_map_concatenation"
fusion_df["case_type"] = add_case_type(fusion_df)
fusion_df["note"] = "CBAM does not use a two-scalar softmax gate. w_raw and w_texture are normalized averages of CBAM channel attention over raw and texture channel groups, saved for branch-emphasis auditing."
...
branch_df["note"] = "This CBAM model has no separate raw-only or texture-only classification heads. Branch-emphasis auditing is provided through cbam_raw_channel_weight and cbam_texture_channel_weight."
