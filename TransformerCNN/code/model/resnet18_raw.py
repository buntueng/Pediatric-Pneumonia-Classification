...
OUTPUT_PATH = "/home/eecommu06/Desktop/Bee/Pediatric_Pneumonia/output/raw_resnet18"

MODEL_NAME = "raw_resnet18"
...
OLD_HISTORY_CSV_PATH = os.path.join(OUTPUT_PATH, "training_history_raw_resnet18.csv")
OLD_RESULTS_CSV_PATH = os.path.join(OUTPUT_PATH, "raw_resnet18_results.csv")
...
glcm_config = {
    ...
    "model_name": MODEL_NAME,
    "note": "Raw-image ResNet-18 baseline does not compute GLCM maps. Fill real values in GLCM/fusion scripts.",
    "image_size": list(IMG_SIZE),
...
model_config = {
    ...
    "classifier": "Dropout(p=0.2) -> Linear(512, num_classes)",
    "fusion": {
        "used_in_this_model": False,
...
for fold, (train_idx, val_idx) in enumerate(kfold.split(indices), start=1):
    ...
    model = ResNet18Model(num_classes=len(base_dataset.classes)).to(DEVICE)
...
complexity_model = ResNet18Model(num_classes=len(base_dataset.classes)).to(DEVICE)
...
if GENERATE_XAI:
    ...
    xai_model = ResNet18Model(num_classes=len(base_dataset.classes)).to(DEVICE)
    ...
    gradcam = GradCAM(xai_model, xai_model.backbone.layer4[-1])
