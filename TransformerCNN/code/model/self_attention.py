...
OUTPUT_PATH = "/home/eecommu06/Desktop/Bee/Pediatric_Pneumonia/output/fusion_self_attn"

MODEL_NAME = "fusion_self_attn_effnetb0_resnet18_glcm"
...
OLD_HISTORY_CSV_PATH = os.path.join(OUTPUT_PATH, "training_history_self_attn.csv")
OLD_RESULTS_CSV_PATH = os.path.join(OUTPUT_PATH, "self_attn_results.csv")
...
class FusionModel(nn.Module):
    def __init__(self, num_classes=2):
        ...
        # Branch 1: Raw CXR feature maps from EfficientNet-B0.
        effnet = models.efficientnet_b0(weights=models.EfficientNet_B0_Weights.DEFAULT)
        self.raw_features = effnet.features
        self.raw_dim = 1280

        # Branch 2: GLCM texture feature maps from ResNet-18.
        resnet = models.resnet18(weights=models.ResNet18_Weights.DEFAULT)
        self.glcm_features = nn.Sequential(*list(resnet.children())[:-2])
        self.texture_dim = 512

        # Spatial feature-map fusion + self-attention.
        self.fusion_dim = self.raw_dim + self.texture_dim
        self.self_attn = SelfAttention(self.fusion_dim)
    ...
    def forward(self, raw, glcm, return_features=False):
        raw_map = self.raw_features(raw)                  # [B, 1280, 7, 7]
        texture_map = self.glcm_features(glcm)            # [B, 512, 7, 7]
        fused_map = torch.cat((raw_map, texture_map), dim=1)
        attended_map, spatial_attention = self.self_attn(fused_map, return_attention=True)
        pooled = self.global_pool(attended_map)
        logits = self.classifier(pooled)

        if return_features:
            raw_vec = self.global_pool(raw_map).flatten(1)
            texture_vec = self.global_pool(texture_map).flatten(1)
            fused_vec = pooled.flatten(1)

            # Self-attention does not produce branch-wise scalar gates. For auditing,
            # summarize branch contribution using feature L2 norm ratios before attention.
            raw_norm = torch.linalg.vector_norm(raw_vec, ord=2, dim=1)
            texture_norm = torch.linalg.vector_norm(texture_vec, ord=2, dim=1)
            denom = raw_norm + texture_norm + 1e-12
            self_attn_raw_weight = raw_norm / denom
            self_attn_texture_weight = texture_norm / denom
            self_attn_spatial_mean = spatial_attention.flatten(1).mean(dim=1)
            self_attn_gamma = self.self_attn.gamma.expand_as(self_attn_spatial_mean)
            ...
            return (
                ...
                texture_vec,
                fused_vec,
                self_attn_raw_weight,
                self_attn_texture_weight,
                self_attn_spatial_mean,
                self_attn_gamma,
            )
        return logits
...
experiment_config = {
    ...
    "augmentation": {
        "used": False,
        "details": "Original self.py used resize + tensor conversion + ImageNet normalization only.",
        "rotation_degrees": 0,
        "translation": 0,
        "brightness_jitter": 0,
        "contrast_jitter": 0,
...
glcm_config = {
    ...
    "compute_glcm_after_resize": "not_specified_in_original_self_py",
    "gray_levels": "not_specified_in_original_self_py",
    "window_size": "not_specified_in_original_self_py",
    "distances": "not_specified_in_original_self_py",
    "angles_degrees": "not_specified_in_original_self_py",
    "offsets": "not_specified_in_original_self_py",
    "direction_aggregation": "not_specified_in_original_self_py",
    "entropy_formula": "not_specified_in_original_self_py",
    "border_handling": "not_specified_in_original_self_py",
...
model_config = {
    ...
    "raw_branch": {
        ...
        "feature_map_dim": 1280,
        "feature_map_size_for_224_input": "7x7",
    },
    "texture_branch": {
        "backbone": "ResNet-18",
    ...
    "fusion": {
        "type": "spatial_feature_map_concatenation_followed_by_self_attention",
        "attention_or_gate_used": True,
        "fusion_map_channels": 1792,
        "self_attention_position": "after concatenating EfficientNet-B0 raw feature maps and ResNet-18 GLCM feature maps, before global average pooling",
        "self_attention": "query/key/value 1x1 convolutions over fused spatial tokens with residual gamma scaling",
        "gate_network": "not_used_two_scalar_gate; spatial self-attention is used instead",
        "fusion_weight_type": "branch feature-norm ratio summary; w_raw and w_texture are normalized raw/texture L2 norms for auditing",
        "se_block_position": "not_applicable_self_attention_used_instead_of_SE",
        "note": "w_raw and w_texture saved in fusion_weights_test.csv are branch feature-norm ratios, not softmax gate outputs.",
    },
    "classifier": "SelfAttention -> AdaptiveAvgPool2d(1) -> Flatten -> Linear(1792,512) -> ReLU -> Dropout(0.3) -> Linear(512,num_classes)",
}
...
def evaluate_with_predictions(model, loader, fold, threshold=0.5):
    ...
    with torch.no_grad():
        for raw, glcm, labels, raw_paths, glcm_paths in loader:
            raw = raw.to(DEVICE)
            glcm = glcm.to(DEVICE)
            outputs, raw_vec, texture_vec, fused_vec, self_attn_raw_w, self_attn_texture_w, self_attn_spatial_mean, self_attn_gamma = model(raw, glcm, return_features=True)
            probs = torch.softmax(outputs, dim=1).detach().cpu().numpy()
            ...
            self_attn_raw_w_np = self_attn_raw_w.detach().cpu().numpy()
            self_attn_texture_w_np = self_attn_texture_w.detach().cpu().numpy()
            self_attn_spatial_mean_np = self_attn_spatial_mean.detach().cpu().numpy()
            self_attn_gamma_np = self_attn_gamma.detach().cpu().numpy()
            ...
            for i in range(len(labels)):
                row = {
                    ...
                    "self_attn_raw_norm_weight": float(self_attn_raw_w_np[i]),
                    "self_attn_texture_norm_weight": float(self_attn_texture_w_np[i]),
                    "self_attn_spatial_attention_mean": float(self_attn_spatial_mean_np[i]),
                    "self_attn_gamma": float(self_attn_gamma_np[i]),
                }
...
fusion_df = predictions_df[[
    ...
    "self_attn_raw_norm_weight", "self_attn_texture_norm_weight", "self_attn_spatial_attention_mean", "self_attn_gamma"
]].copy()
fusion_df["w_raw"] = fusion_df["self_attn_raw_norm_weight"]
fusion_df["w_texture"] = fusion_df["self_attn_texture_norm_weight"]
fusion_df["fusion_type"] = "self_attention_after_spatial_feature_map_concatenation"
fusion_df["case_type"] = add_case_type(fusion_df)
fusion_df["note"] = "Self-attention does not use a two-scalar softmax gate. w_raw and w_texture are branch feature-norm ratios, saved for branch-behavior auditing."
...
branch_df["note"] = "This self-attention model has no separate raw-only or texture-only classification heads. Branch-behavior auditing is provided through self_attn_raw_norm_weight and self_attn_texture_norm_weight."
...
if GENERATE_XAI:
    ...
    raw_gradcam = DualInputGradCAM(xai_model, xai_model.raw_features[-1])
    glcm_gradcam = DualInputGradCAM(xai_model, xai_model.glcm_features[-1][-1])
