...
OUTPUT_PATH = "/home/eecommu06/Desktop/Bee/Pediatric_Pneumonia/output/fusion"

MODEL_NAME = "fusion_concat_effnetb0_resnet18_glcm"
...
class FusionModel(nn.Module):
    def __init__(self, num_classes=2):
        ...
        # Standard feature concatenation + MLP classifier.
    ...
    def forward(self, raw, glcm, return_features=False):
        ...
        fused_vec = torch.cat((raw_vec, texture_vec), dim=1)
        logits = self.mlp_classifier(fused_vec)
        if return_features:
            return logits, raw_vec, texture_vec, fused_vec
        return logits
...
experiment_config = {
    ...
    "augmentation": {
        "used": False,
        "details": "Original fusion.py used resize + tensor conversion + ImageNet normalization only.",
        "rotation_degrees": 0,
        "translation": 0,
        "brightness_jitter": 0,
        "contrast_jitter": 0,
...
glcm_config = {
    ...
    "compute_glcm_after_resize": "not_specified_in_original_fusion_py",
    "gray_levels": "not_specified_in_original_fusion_py",
    "window_size": "not_specified_in_original_fusion_py",
    "distances": "not_specified_in_original_fusion_py",
    "angles_degrees": "not_specified_in_original_fusion_py",
    "offsets": "not_specified_in_original_fusion_py",
    "direction_aggregation": "not_specified_in_original_fusion_py",
    "entropy_formula": "not_specified_in_original_fusion_py",
    "border_handling": "not_specified_in_original_fusion_py",
...
model_config = {
    ...
    "fusion": {
        "type": "direct_feature_concatenation",
        "attention_or_gate_used": False,
        "fusion_vector_dim": 1792,
        "gate_network": None,
        "fusion_weight_type": "not_applicable_no_gate_or_attention",
        "se_block_position": "not_applicable_SE_not_used_in_this_standard_fusion_script",
        "note": "w_raw and w_texture are not learned in this model. Use the SE/gated model script to extract real fusion weights.",
    },
...
def evaluate_with_predictions(model, loader, fold, threshold=0.5):
    ...
    with torch.no_grad():
        for raw, glcm, labels, raw_paths, glcm_paths in loader:
            raw = raw.to(DEVICE)
            glcm = glcm.to(DEVICE)
            outputs, raw_vec, texture_vec, fused_vec = model(raw, glcm, return_features=True)
            probs = torch.softmax(outputs, dim=1).detach().cpu().numpy()
...
fusion_df["w_raw"] = np.nan
fusion_df["w_texture"] = np.nan
fusion_df["fusion_type"] = "direct_concatenation_no_learned_weights"
fusion_df["case_type"] = add_case_type(fusion_df)
fusion_df["note"] = "This concat model has no gate/SE attention; w_raw and w_texture are not generated. Feature-norm ratios are saved only as auditing descriptors."
